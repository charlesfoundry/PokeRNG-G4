...
const gen4EggPickupMaxSearchStates = 4000000;

class EggPickupSearchRequest {
  const EggPickupSearchRequest({
    this.seed,
    this.year,
    this.minDelay,
    this.maxDelay,
  ...
  bool get canSearch {
    ...
    return (hasSeed || hasTimeRange) &&
        ...
        resultLimit > 0 &&
        totalSearchStates > 0 &&
        totalSearchStates <= gen4EggPickupMaxSearchStates;
  }
}

class EggPickupSearchResult {
  const EggPickupSearchResult({
    required this.seed,
    required this.hour,
...
class EggPickupSearchResponse {
  const EggPickupSearchResponse({
    required this.results,
    required this.progress,
    required this.resultLimitReached,
  });

  final List<EggPickupSearchResult> results;
  final Gen4SearchProgress progress;
  final bool resultLimitReached;
}

class EggPickupSearchJob {
  EggPickupSearchJob._({
    required ReceivePort receivePort,
    required void Function(Gen4SearchProgress progress) onProgress,
    required void Function(EggPickupSearchResponse result) onComplete,
  ...
  final void Function(EggPickupSearchResponse result) _onComplete;
  final void Function(String error) _onError;
  Isolate? _isolate;
  bool _cancelled = false;

  static Future<EggPickupSearchJob> start({
    required EggPickupSearchRequest request,
    required void Function(Gen4SearchProgress progress) onProgress,
    required void Function(EggPickupSearchResponse result) onComplete,
    required void Function(String error) onError,
  }) async {
    final receivePort = ReceivePort();
    final job = EggPickupSearchJob._(
    ...
    job._isolate = await Isolate.spawn(
      _runEggPickupSearch,
      _EggPickupSearchMessage(sendPort: receivePort.sendPort, request: request),
    );
    job._listen();
    return job;
  }
    ...
    _receivePort.listen((message) {
      ...
      switch (message) {
        case Gen4SearchProgress():
          _onProgress(message);
        case EggPickupSearchResponse():
          _finish();
          _onComplete(message);
        case _EggPickupSearchFailure():
          _finish();
...
class _EggPickupSearchMessage {
  const _EggPickupSearchMessage({
    required this.sendPort,
    required this.request,
  });

  final SendPort sendPort;
  final EggPickupSearchRequest request;
}

class _EggPickupSearchFailure {
  const _EggPickupSearchFailure(this.error);

  final String error;
}

void _runEggPickupSearch(_EggPickupSearchMessage message) {
  try {
    message.sendPort.send(
      searchEggPickupsSync(message.request, onProgress: message.sendPort.send),
    );
  } catch (error) {
    message.sendPort.send(_EggPickupSearchFailure(error.toString()));
  }
}

EggPickupSearchResponse searchEggPickupsSync(
  EggPickupSearchRequest request, {
  ...
  final results = <EggPickupSearchResult>[];
  ...
  return EggPickupSearchResponse(
...
EggPickupSearchResult _resultFromState(
  int seed,
  Gen4EggState state, {
  int? displayDelay,
}) {
  return EggPickupSearchResult(
    seed: seed,
    hour: (seed >>> 16) & 0xff,
