...
class EggPickupResultsPage extends StatefulWidget {
  const EggPickupResultsPage({super.key, required this.request});

  final EggPickupSearchRequest request;

  @override
  State<EggPickupResultsPage> createState() => _EggPickupResultsPageState();
}

class _EggPickupResultsPageState extends State<EggPickupResultsPage> {
  EggPickupSearchJob? _job;
  Gen4SearchRunState _state = Gen4SearchRunState.running;
  Gen4SearchProgress? _progress;
  List<EggPickupSearchResult> _results = const [];
  ...
  Future<void> _startSearch() async {
    try {
      _job = await EggPickupSearchJob.start(
...
class _EggPickupResultCard extends StatelessWidget {
  ...
  final EggPickupSearchResult result;
